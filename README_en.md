...
- Header
  ...
  - Provides a language switcher in the top-right corner: `日本語` / `English` / `简体中文` / `繁體中文`
  - Includes `Label Management`, `Show Meta`, `Shortcuts`, and a list toggle for mobile layouts.
  - `Show Meta` is hidden by default. It lets you check `session root`, `path`, `cwd`, `time`, `source`, `events`, and `raw lines`.
- Left pane: session list
  - Displays a session preview, `source` labels (`CLI` / `VS Code`), session labels, and `cwd`.
  - Shows the `sessions: filtered/total` count above the list.
  - Lets you change the sort order with the `Newest`, `Oldest`, and `Last Updated` tabs.
  - `Clear` resets the search conditions and filters in the left pane.
  - `Show Filters` / `Hide Filters` lets you collapse or expand the search and filter area.
  - In portrait layout, `Hide List` / `Show List` in the header toggles the entire left pane.
- Search and filters in the left pane
  - Filters by `cwd` / `Start Date` / `End Date` / `Event Start Datetime` / `Event End Datetime` / keyword / `source` / session label / event label.
  - The time input for event datetime becomes enabled after you enter the corresponding date.
  - The search targets include not only `message`, but also `function_call.arguments`, `function_output.output`, and `agent_update.message`.
  ...
  - The `AND/OR` switch applies only to the keyword field.
    - `AND`: includes sessions containing all space-separated keywords.
    - `OR`: includes sessions containing any of the space-separated keywords.
- Right pane: chronological event view for the selected session
  - Shows a loading indicator during the initial detail load, and an updating overlay during manual `Refresh`.
  - Uses a flat detail toolbar with `Display`, `Actions`, `Search`, and `Range Selection`.
  - `Show Detail Actions` / `Hide Detail Actions` toggles the `Actions`, `Search`, and `Range Selection` sections together.
  - Display, search, and range-selection controls are disabled when no session is selected.
- Display and actions in the right pane
  - Display conditions: `Only User Instructions` / `Only AI Responses` / `Only Each Input and Final Reply` / `Reverse Order` / `label`
  ...
  - `Clear` resets the entire state of the right pane.
    ...
    - Anchor selection mode, selected anchor, and show-before/show-after anchor state
...
- Search and selection in the right pane
  ...
  - Detail keywords use direct substring matching as entered, not AND / OR parsing.
  - The search targets are `message` / `function_call` / `function_output` / `agent_update`.
  - Pressing `Enter` in the search field runs the search, then removes focus so you can move with `N` / `P`.
  - `Event Start Datetime` / `Event End Datetime` can narrow the event timeline displayed in the right pane.
  - The event datetime filter in the right pane also uses separate `date + time` inputs, and the time field becomes enabled after a date is entered.
  - In `Selection Mode`, you can check each `message` and copy them together with `Copy Selected`.
    - Already selected `message` items remain selected even while a filter is applied.
  - In `Anchor Selection Mode`, you can select a single `message` and filter with `Show Only After Anchor` / `Show Only Before Anchor`.
- Event display
...
- Label Management
  ...
  - Label colors can be entered directly as `#hex` / `rgb(...)` / `oklch(...)`, or selected from preset colors.
  - In add-label UIs, candidates remain colorized so they are easier to identify.
