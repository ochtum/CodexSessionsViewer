...
const I18N = {
  ja: {
    ...
    'header.subtitle': 'CodexCLIのイベント履歴を一覧表示・詳細表示して、検索できます。\n覚えておきたい内容にラベルを付けて、あとから見つけることもできます。',
    ...
    'meta.time': 'time',
    'meta.status': 'status',
    'meta.reasoningLevel': 'reasoning level',
    'meta.fxRate': 'fx rate',
    'meta.usage': 'usage',
    'meta.models': 'models',
    ...
    'usage.score': 'score',
    'usage.rank': 'rank',
    'usage.tooltip.input': 'モデルに送られた入力トークン数です。cache 再利用分を含みます。',
    'usage.tooltip.cached': '入力トークンのうち、cache から再利用されたトークン数です。',
    'usage.tooltip.output': 'モデルが生成した出力トークン数です。',
    'usage.tooltip.reasoning': '推論用として記録された出力トークン数です。',
    'usage.tooltip.total': '入力と出力を合計した総トークン数です。',
    'usage.tooltip.cost': 'この usage をもとに価格表から見積もった概算コストです。表示通貨を選択している場合は換算額も併記します。',
    'usage.tooltip.perDollar': '概算コスト 1 ドルあたりの総トークン数です。',
    'usage.tooltip.score': '総トークン数 / 概算コスト(USD) の常用対数です。1.0 上がるとコスパは 10 倍です。',
  ...
  en: {
    ...
    'header.subtitle': 'Browse CodexCLI event histories in list and detail views, and search them.\nYou can also attach labels to anything worth remembering and find it later.',
    ...
    'usage.score': 'score',
    'usage.rank': 'rank',
    'usage.tooltip.input': 'Number of input tokens sent to the model, including cached reuse.',
    'usage.tooltip.cached': 'Number of input tokens reused from cache.',
    'usage.tooltip.output': 'Number of output tokens generated by the model.',
    'usage.tooltip.reasoning': 'Number of output tokens reported as reasoning tokens.',
    'usage.tooltip.total': 'Total number of input and output tokens combined.',
    'usage.tooltip.cost': 'Estimated USD cost calculated from this usage and the pricing catalog. If a display currency is selected, a converted amount is also shown.',
    'usage.tooltip.perDollar': 'Total tokens processed per estimated US dollar.',
    'usage.tooltip.score': 'Base-10 logarithm of total_tokens / estimated_cost_usd. A +1.0 increase means 10x better efficiency.',
  ...
  'zh-Hans': {
    ...
    'header.subtitle': '可以通过列表和详细视图查看 CodexCLI 的事件历史，并进行搜索。\n还可以给想保留的内容加上标签，之后再轻松找到。',
    ...
    'usage.score': '评分',
    'usage.rank': '等级',
    'usage.tooltip.input': '发送给模型的输入 token 数，包含缓存复用部分。',
    'usage.tooltip.cached': '输入 token 中从缓存复用的 token 数。',
    'usage.tooltip.output': '模型生成的输出 token 数。',
    'usage.tooltip.reasoning': '被记录为推理 token 的输出 token 数。',
    'usage.tooltip.total': '输入与输出合计的总 token 数。',
    'usage.tooltip.cost': '根据这次 usage 和价格表估算出的成本。若选择了显示货币，也会同时显示换算金额。',
    'usage.tooltip.perDollar': '按估算成本换算，每 1 美元对应的总 token 数。',
    'usage.tooltip.score': '总 token 数 / 预估成本(USD) 的常用对数。每增加 1.0，成本效益提升 10 倍。',
...
I18N['zh-Hant'] = {
  ...
  'header.subtitle': '可以透過列表與詳細檢視查看 CodexCLI 的事件歷史，並進行搜尋。\n還可以替想保留的內容加上標籤，之後再輕鬆找到。',
  ...
  'usage.score': '評分',
  'usage.rank': '等級',
  'usage.tooltip.input': '傳送給模型的輸入 token 數，包含快取重用部分。',
  'usage.tooltip.cached': '輸入 token 中從快取重用的 token 數。',
  'usage.tooltip.output': '模型產生的輸出 token 數。',
  'usage.tooltip.reasoning': '被記錄為推理 token 的輸出 token 數。',
  'usage.tooltip.total': '輸入與輸出合計的總 token 數。',
  'usage.tooltip.cost': '根據這次 usage 與價格表估算出的成本。若選擇了顯示貨幣，也會同時顯示換算金額。',
  'usage.tooltip.perDollar': '按估算成本換算，每 1 美元對應的總 token 數。',
  'usage.tooltip.score': '總 token 數 / 預估成本(USD) 的常用對數。每增加 1.0，成本效益提升 10 倍。',
...
function applyMainLanguage(){
  ...
  document.title = 'Codex Sessions Viewer';
...
function getRequestedCostCurrency(){
  const params = new URLSearchParams(window.location.search);
  return normalizeCostCurrency(params.get('currency'))
    || normalizeCostCurrency(readStoredCostCurrency())
    || getDefaultCostCurrencyForLanguage(uiLanguage);
}
...
function openCostSummaryWindow(){
  ...
  window.open(`/costs?${params.toString()}`, 'codex_cost_summary', features);
}
...
function formatLocalCurrency(value, currencyCode){
  const n = Number(value);
  if(!Number.isFinite(n) || !currencyCode || currencyCode === 'USD') return '';
  if(currencyCode === 'JPY') return formatJpy(n);
  if(currencyCode === 'CNY') return formatCny(n);
  const digits = n >= 10 ? 2 : (n >= 1 ? 3 : 4);
  return new Intl.NumberFormat(getUiLocale(), {
    style: 'currency',
...
function renderTodayUsageSummary(){
  ...
  const metrics = [
    ...
    {
      label: t('usage.cost'),
      value: formatUsageCostDisplay(summary.cost_usd, state.todayUsageExchangeRate),
...
function formatTokenUsageEventBody(ev){
  ...
  if(ev.model){
    ...
    lines.push(`${t('usage.model')}: ${modelText}`);
...
function isRangeSelectableMessageEvent(ev){
  if(!getEventSelectionKey(ev)){
    return false;
  }
  if(ev && ev.kind === 'token_usage'){
    return true;
  }
  return isCopyableMessageEvent(ev);
}
...
function renderUsageMetaRows(usage){
  if(!usage) return '';
  const performance = getUsageCostPerformance(usage.total_tokens || 0, usage.cost_usd);
  const metrics = [
    {
      label: t('usage.input'),
  ...
  if(models.length){
    const reasoningTag = reasoningLevel
      ? `<span class="meta-tag">${esc(`${t('meta.reasoningLevel')}:`)}</span><span class="header-meta-text">${esc(reasoningLevel)}</span>`
      : '';
    modelsRow = `<div class="header-meta-row"><span class="header-meta-label">${esc(t('meta.models'))}</span><span class="header-meta-value">${esc(models.join(', '))}</span>${reasoningTag}</div>`;
  } else if(reasoningLevel){
    modelsRow = `<div class="header-meta-row"><span class="header-meta-label">${esc(t('meta.reasoningLevel'))}</span><span class="header-meta-value">${esc(reasoningLevel)}</span></div>`;
  }
  ...
  const fxRow = fxDisplay
    ...
    : '';
  return usageRow + modelsRow + fxRow;
}
...
function initViewerPage(){
  ...
  document.getElementById('events').addEventListener('change', (event) => {
    ...
    if(target.classList.contains('event-range-radio') && target.checked){
      updateMessageRangeSelection(target.dataset.eventId);
      return;
    }
  });
  ...
  document.getElementById('open_cost_summary').addEventListener('click', openCostSummaryWindow);
  const currencySelect = document.getElementById('currency_select');
  if(currencySelect){
