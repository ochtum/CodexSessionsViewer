...
const I18N = {
  ja: {
    ...
    'usage.tooltip.cost': 'この usage をもとに価格表から見積もった概算コスト（USD）です。',
  ...
  en: {
    ...
    'usage.tooltip.cost': 'Estimated USD cost calculated from this usage and the pricing catalog.',
  ...
  'zh-Hans': {
    ...
    'usage.tooltip.cost': '根据这次 usage 和价格表估算出的成本（USD）。',
...
I18N['zh-Hant'] = {
  ...
  'usage.tooltip.cost': '根據這次 usage 與價格表估算出的成本（USD）。',
...
function openCostSummaryWindow(){
  ...
  window.open(`/costs?lang=${encodeURIComponent(uiLanguage)}`, 'codex_cost_summary', features);
}
